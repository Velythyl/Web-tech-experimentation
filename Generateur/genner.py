...
with open("ca-500.csv") as csv_file:
    ...
    for row in csv_reader:
        login = row[0]+str(line_count)

        if(row[1] in noms): continue
        if(row[0] in prenoms): continue
        if(login in logins): continue
        ...
        individus.append([line_count, row[1], row[0], ''.join(choice(string.ascii_uppercase + string.digits) for _ in range(10)), login])
...
with open("./genned_csv/plages.csv", "w", newline='') as f:
    writer = csv.writer(f, lineterminator='\n')
    writer.writerows(plage)
